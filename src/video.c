...
void initvideo()
{
        ...
        if (colourboard) set_palette(atompal);
        else             set_palette(monopal);
        set_color_depth(depth);        
}
                ...
                switch (gfxmode)
                {
                        ...
                        for (x=0;x<256;x+=8)
                        {
                                ...
                                if (chr&0x40)
                                {
                                        ...
                                        chr<<=((sy>>2)<<1);
                                        chr=(chr>>4)&3;
                                        if (chr&2) col=semigrcol[(temp>>6)|(css<<1)];
                                        else       col=0;
                                        b->line[line][x]=b->line[line][x+1]=b->line[line][x+2]=b->line[line][x+3]=col;
                                        if (chr&1) col=semigrcol[(temp>>6)|(css<<1)];
                                        else       col=0;
...
//                        exit(-1);
        ...
        if (line==192)
        {
                ...
                {
                        savescrshot=0;
                        if (colourboard) save_bmp(scrshotname,b,atompal);
                        else             save_bmp(scrshotname,b,monopal);
                }
                ...
                if ((!(tapeon && fasttape) && fskipcount>=fskipmax) || frmcount==60)
                {
                        ...
                        if (fullscreen)
                        {
                                ...
                                if (tapeon) rectfill(screen,1000,0,1023,8,makecol(255,0,0));
                        }
                        else
                        {
                                stretch_blit(b2,screen,0,0,256,192,0,0,winsizex,winsizey);
                                if (tapeon) rectfill(screen,winsizex-12,0,winsizex,4,makecol(255,0,0));
                        }
                        frmcount=0;
                }
                endblit();
        ...
        if ((line==261 && !colourboard) || line==311)
        {
                switch (gfxmode)
                {
                        ...
                        for (x=0;x<32;x++) fetcheddat[x]=ram[0x8000+x];
                        break;
                        case 1: case 3: case 5: case 7: case 11: /*16-byte per line*/
                        for (x=0;x<32;x++) fetcheddat[x]=ram[0x8000+(x>>1)];
                        break;
                }
        }
