...
)
{
	...
	if (res == FR_OK) {		/* Initialize the SFN entry */
		...
		if (res == FR_OK) {
			...
			mem_cpy((void*)dir, (const void*)dj->fn, 11);	/* Put SFN */
...
/* Return info about file using the global DIR struct 'dj' */
	...
	for ( ;  btr;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {			/* On the sector boundary? */
			...
			if (cc) {								/* Read maximum contiguous sectors directly */
               ...
               mem_cpy((void*)(rbuff + ((fp->fs->winsect - sect) * SS(fp->fs))), (void*)fp->fs->win, SS(fp->fs));
#else
		...
		if (move_window(fp->fs, fp->dsect))			/* Move sector window */
			ABORT(fp->fs, FR_DISK_ERR);
		mem_cpy((void*)rbuff, (const void*)&fp->fs->win[fp->fptr % SS(fp->fs)], rcnt);	/* Pick partial sector */
#else
...
)
{
   ...
   DIR sdj;
	...
	if (dir[DIR_Attr] & AM_DIR) {			/* It is a sub-directory */
		...
		mem_cpy((void*)&sdj, (const void*)&dj, sizeof(DIR));	/* Check if the sub-dir is empty or not */
		sdj.sclust = dclst;
		res = dir_seek(&sdj, 2);
		if (res != FR_OK) LEAVE_FF(dj.fs, res);
		res = dir_read(&sdj);
...
)
{
   ...
   mem_cpy((void*)(dir+32), (const void*)dir, 32);         /* Create ".." entry */
	dir[33] = '.';
	pclst = dj.sclust;
...
)
{
   ...
   mem_cpy((void*)buf, (const void*)(dj_old.dir+DIR_Attr), 21);     /* Save the object information */

	mem_cpy((void*)&dj_new, (const void*)&dj_old, sizeof(DIR));
	...
	if (res == FR_NO_FILE) { 					/* Is it a valid path and no name collision? */
		...
		if (res == FR_OK) {
         ...
         mem_cpy((void*)(dir+13), (const void*)(buf+2), 19);
