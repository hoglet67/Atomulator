...
//                printf("Keyrow now %i %04X\n",keyrow,pc);
                ...
                switch (val&0xE)
                {
                        case 0x4: speaker=val&1;                 rpclog("Speaker %i\n",(val&4)>>2); break;
                        case 0x6: css=(val&1)?2:0; break;
                }
                break;
...
//        printf("Read 8255 %04X %04X\n",addr,pc);
        ...
        {
                case 0:
                return keyrow|(gfxmode<<4);
                case 1:
                for (c=0;c<128;c++)
                {
                        if (key[c] && keyl[keylookup[c]]&0x80 && keyrow==(keyl[keylookup[c]]&0xF)) temp&=~(1<<((keyl[keylookup[c]]&0x70)>>4));
                        if (key[c] && keylookup[c]==KEY_LCONTROL) temp&=~0x40;
                        if (key[c] && (keylookup[c]==KEY_LSHIFT || keylookup[c]==KEY_RSHIFT)) temp&=~0x80;
                }
...
//                if (key[keylookup[KEY_LSHIFT]] || key[keylookup[KEY_RSHIFT]]) temp&=~0x80;
                ...
                case 2:
                if (vbl) temp&=~0x80;
                if (key[KEY_ALT] || key[KEY_ALTGR]) temp&=~0x40;
                if (!css) temp&=~8;
                if (!speaker) temp&=~4;
                if (!intone)  temp&=~0x10;
                if (!tapedat) temp&=~0x20;
//                printf("VBL %i %04X\n",vbl,pc);
                return temp;
//                default:
...
void polltape()
{
        ...
        else
        {
                ...
                if (tapeon)
                {
                        ...
                        {
                                if (databyte&1) tapedat=bitvalid&1;
                                else            tapedat=bitvalid&2;
                                bitvalid--;
                                if (!bitvalid)
                                {
                                        bytevalid--;
...
{
        int16_t temp=0;
        if (spon) temp+=(speaker)?4095:-4096;
        if (tpon) temp+=(tapedat)?2047:-2048;
