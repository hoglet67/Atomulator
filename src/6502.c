...
void initmem()
{
        ...
        rom=(uint8_t *)malloc(0xA000);
        memset(ram,0,0x10000);
        for (c=0;c<0x100;c++) memstat[c]=2;
        ram[8]=rand()&255;
        ram[9]=rand()&255;
        ram[10]=rand()&255;
        ram[11]=rand()&255;
...
void loadroms()
{
        FILE *f=fopen("roms/akernel.rom","rb");
        fread(rom,4096,1,f);
        fclose(f);
        f=fopen("roms/abasic.rom","rb");
        fread(rom+4096,4096,1,f);
        fclose(f);
        f=fopen("roms/afloat.rom","rb");
        fread(rom+8192,4096,1,f);
        fclose(f);
        f=fopen("roms/dosrom.rom","rb");
        fread(rom+16384,4096,1,f);
        fclose(f);
        f=fopen("roms/axr1","rb");
        fread(rom+0x3000,4096,1,f);
        fclose(f);
        f=fopen("roms/atom_bbc_basic_os.rom","rb");
        fread(rom+0x5000,4096,1,f);
        fclose(f);
        f=fopen("roms/basic1.rom","rb");
        fread(rom+0x6000,16384,1,f);
        fclose(f);
}
...
uint8_t readmeml(uint16_t addr)
{
        ...
        if (!bbcmode)
        {
                switch (addr&0xFC00)
                {
                        ...
                        if (snow && cycles>=0 && cycles<32) fetcheddat[31-cycles]=ram[addr];
                        return ram[addr];
                        case 0x0800:
                        if ((addr&~0x7F)==0xA00) return read8271(addr); /*FDC*/
                        break;
        ...
        else
        {
                switch (addr&0xFC00)
                {
                        ...
                        return rom[(addr&0x3FFF)+0x6000]; /*Not implemented*/
                        case 0xF000: case 0xF400: case 0xF800: case 0xFC00: /*Kernel*/
                        return rom[(addr&0xFFF)|0x5000]; /*Not implemented*/
                }
        }
        return 0;
...
void writememl(uint16_t addr, uint8_t val)
{
        ...
        if (!bbcmode)
        {
                switch (addr&0xFC00)
                {
                        ...
                        case 0x8000: case 0x8400: case 0x8800: case 0x8C00: /*Video RAM*/
                        case 0x9000: case 0x9400: case 0x9800: case 0x9C00:
                        if (snow && cycles>=0 && cycles<32) fetcheddat[31-cycles]=val;
                        ram[addr]=val;
                        return;
                        case 0x0800:
                        if ((addr&~0x7F)==0xA00) { write8271(addr,val); return; } /*FDC*/
                        break;
                        case 0xB000: /*8255 PIA*/
                        write8255(addr,val);
                        return;
                        case 0xB800: /*6522 VIA*/
                        if (addr<0xB810) writevia(addr,val);
...
//                rpclog("Exec line %i\n",lines);
                lns=lines;
                if (lines<262 || lines==311) drawline(lines);
                pollsound();
                cycles+=cpl;
//                badline=0;
                while (cycles>0)
...
//                        if (pc==0x8000) a=1;
                        if (pc==0xC2CF || pc<0x8000) tapeon=0;
                        if (pc==0xFB8E) tapeon=1;
/*                        if (pc==0xFC23)
                        {
...
//                                rpclog("NMI %04X\n",pc);
                        ...
                        if (motoron)
                        {
                                if (fdctime) { fdctime-=oldcyc; if (fdctime<=0) fdccallback(); }
                                disctime-=oldcyc; if (disctime<=0) { disctime+=8; disc_poll(); }
                        }
