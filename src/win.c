...
void catupdatewindow()
{
        int c;
        if (!catwindowopen) return;
        for (c=0;c<catnum;c++) SendMessage(cath,LB_ADDSTRING,(int)NULL,(int)catnames[c]);
}
...
//                SendMessage(h,LB_ADDSTRING,NULL,"test");
                ...
                {
                        case IDCANCEL: case IDOK: return TRUE;
                }
                break;
                case WM_SYSCOMMAND:
