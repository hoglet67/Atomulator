...
uint8_t dreadmem(uint16_t addr)
{
        if (!bbcmode)
        {
                if (addr>=0xA00 && addr<0xAFF) return 0xFF;
                if (addr>=0xB000 && addr<0xBFFF) return 0xFF;
        }
        else
        {
                if (addr>=0x7000 && addr<0x7FFF) return 0xFF;
        }
        return readmem(addr);
}
enum
...
//                        FreeConsole();
                        ...
                        return;
                        case 'm': case 'M':
                        if (params) sscanf(&ins[d],"%X",(unsigned int *)&debugmemaddr);
                        for (c=0;c<16;c++)
                        {
                        ...
                        break;
                        case 'd': case 'D':
                        if (params) sscanf(&ins[d],"%X",(unsigned int *)&debugdisaddr);
                        for (c=0;c<12;c++)
                        {
                                debugout("    ");
                                debugdisassemble();
